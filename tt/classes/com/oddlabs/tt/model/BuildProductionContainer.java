...
public strictfp class BuildProductionContainer extends BuildSupplyContainer {
    ...
    public BuildProductionContainer(
            int max_supply_count,
    ...
    public void orderSupply(int amount, boolean infinite) {
        ...
        if (infinite) super.orderSupply(BuildSpinner.INFINITE_LIMIT - getNumSupplies(), amount);
        else super.orderSupply(amount);
    }

    public final boolean hasEnoughSupplies() {
        for (int i = 0; i < cost.getSupplyTypes().length; i++) {
            if (building.getSupplyContainer(cost.getSupplyTypes()[i]).getNumSupplies()
                    < cost.getSupplyAmounts()[i]) {
                return false;
            }
        }
        return true;
    ...
    public final void build(float delta) {
        ...
        if (man_seconds >= man_seconds_per_production) {
            ...
            if (!dest_container.isSupplyFull()) {
                for (int i = 0; i < cost.getSupplyTypes().length; i++) {
                    building.getSupplyContainer(cost.getSupplyTypes()[i])
                            .increaseSupply(-cost.getSupplyAmounts()[i]);
                }
                if (!infinite) increaseSupply(-1);

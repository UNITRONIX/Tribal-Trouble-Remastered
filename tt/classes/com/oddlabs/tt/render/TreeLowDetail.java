...
public final strictfp class TreeLowDetail {
    private static final StrictVector4f src = new StrictVector4f();
    private static final StrictVector4f dest = new StrictVector4f();
                ...
                new Texture[] {
                    (Texture)
                            Resources.findResource(
                                    new TextureFile(
                                            "/textures/models/lowdetail_tree",
                                            Globals.COMPRESSED_RGBA_FORMAT)),
                    (Texture)
                            Resources.findResource(
                                    new TextureFile(
                                            "/textures/models/viking_lowdetail_tree",
                                            Globals.COMPRESSED_RGBA_FORMAT))
                };
        int[] num_trees;
        switch (terrain_type) {
    ...
    final void build(AbstractTreeGroup tree_root) {
        ...
        assert visitor.end == index_count
                : "end index " + visitor.end + " != num coords " + index_count;
        assert current_vertex_index == vertices.capacity() / 3
                : "vertex index index "
    ...
    private final int numTreesTotal(int[] num_trees) {
        int count = 0;
        for (int i = 0; i < num_trees.length; i++) count += num_trees[i];
        return count;
    }
    ...
    private final int putCoordinate(
            int index,
            float x,
            float y,
            float z,
        ...
        public final void visitTree(TreeSupply tree_supply) {
            ...
            int[] values =
                    putLowDetail(
                            ...
                            tree_supply.getMatrix(),
                            low_details[tree_type_index],
                            vertex_array,
                            texcoord_array,
                            tree_index_array);

...
public final strictfp class PointerInput {
    ...
    static {
        ...
        GLIntImage img_32_8 =
                new GLIntImage(
                        image_32_8.getWidth(),
                        image_32_8.getHeight(),
                        image_32_8.getPixels(),
                        GL11.GL_RGBA);
    }

    public static final void setActiveCursor(Cursor cursor) {
    ...
    public static final void deletingCursor(Cursor cursor) {}

    private static final void updateMouse(GUIRoot gui_root, int x, int y, int dz) {
        ...
        if (dz != 0) LocalInput.getLocalInput().mouseScrolled(gui_root, dz);
    }

    public static final void poll(GUIRoot gui_root) {
        Deterministic deterministic = LocalEventQueue.getQueue().getDeterministic();
        if (deterministic.log(!Mouse.isCreated())) return;
        Mouse.poll();
        int accum_x = last_x;
        int accum_y = last_y;
        int accum_dz = 0;

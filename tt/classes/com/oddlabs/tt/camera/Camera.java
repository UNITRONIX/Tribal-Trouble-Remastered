...
public abstract strictfp class Camera implements Animated {
    ...
    public final void updateChecksum(StateChecksum checksum) {
        // System.out.println("camera_x = " + camera_x + " | camera_y = " + camera_y + " | camera_z
        // = " + camera_z + " | dir_x = " + dir_x + " | dir_y = " + dir_y + " | dir_z = " + dir_z);
    ...
    protected final void checkPosition() {
        ...
        if (!bounce(state.getTargetX(), state.getTargetY(), state.getTargetZ())) {
            if (state.getTargetZ() > GameCamera.MAX_Z) state.setTargetZ(GameCamera.MAX_Z);
        }
    }
    ...
    protected final boolean bounce(float x, float y, float z) {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                float inv_length =
                        LANDSCAPE_OFFSET
                                / (float) StrictMath.sqrt(dx1 * dx1 + dy1 * dy1 + dz1 * dz1);
                dx1 *= inv_length;
                dy1 *= inv_length;
                dz1 *= inv_length;
                ...
                float min_height =
                        StrictMath.max(
                                heightmap.getNearestHeight(x + dx1, y + dy1),
